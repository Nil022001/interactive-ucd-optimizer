...
# ------------------------------------------------------------
# OUTPUT: NEW app-ready baseline file (DO NOT OVERWRITE OLD)
# ------------------------------------------------------------
OUT_PARQUET = "data/pixels_baseline_2019_dt.parquet"
...
# ------------------------------------------------------------
# KEEP COLUMNS needed for the NEW dT_cool + setpoint-aware app
# (still keeps targets for sanity checks)
# ------------------------------------------------------------
keep_cols = [
    "pixel_group", "iy", "ix", "lon", "lat",

    # NEW thermal drivers (already present in your Block A CSV)
    "Tout_C", "Tset_C", "dT_cool",

    # other predictors
    "RH",
    "pop", "infil", "bvol", "bldh", "ahem",

    # keep targets (optional but useful for debug/training)
    "y_ucd_wm2", "y_wpp",
]
...
if missing:
    raise ValueError(
        f"Missing columns in Block A file: {missing}\n"
        "Tip: Your Block A CSV snippet shows Tout_C, Tset_C, dT_cool exist. "
        "If not, regenerate Block A using your updated ML pipeline."
    )
